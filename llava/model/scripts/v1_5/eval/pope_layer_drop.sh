#!/bin/bash
# ============================================================
# 实验 1: Layer-wise Token Drop — POPE
# 一次运行得到两个指标:
#   1. JS divergence vs drop_layer
#   2. POPE accuracy vs drop_layer
# ============================================================
set -e
...
RESULT_DIR=./playground/data/eval/pope/answers_layer_drop
...
" > $ANSWERS_FILE
    ...
    echo "--- POPE score for drop_layer=$DL ---"
    python llava/eval/eval_pope.py \
        --annotation-dir $ANNOTATION_DIR \
        --question-file $QUESTION_FILE \
        --result-file $ANSWERS_FILE \
        | tee ${RESULT_DIR}/pope_score_drop${DL}.txt
...
{
echo "========== Results Summary (POPE Layer Drop) =========="
echo ""

echo "==================== 汇总表格 ===================="
printf "%-12s | %-14s | %-10s | %-14s\n" "drop_layer" "JS Div(mean)" "JS Div(std)" "POPE Acc"
printf "%s\n" "-------------|----------------|------------|---------------"
...
for dl in layers:
    ...
    acc = '-'
    score_file = '${RESULT_DIR}/pope_score_drop' + dl + '.txt'
