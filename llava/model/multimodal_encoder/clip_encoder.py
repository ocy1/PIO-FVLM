...
class CLIPVisionTower(nn.Module):
    ...
    def feature_select(self, image_forward_outs):
        image_features = image_forward_outs.hidden_states[self.select_layer] # [1, 577, 1024]
        image_attentions = image_forward_outs.attentions # [1, 16, 577, 577]
        if self.select_feature == 'patch':
            image_features = image_features[:, 1:]
            image_attentions = torch.stack(image_attentions, dim=0) # [1, 16, 577, 577]
            image_attentions = image_attentions[:, :, :, 0, 1:] # [1, 16, 577]
            # image_attentions_x = image_attentions.unsqueeze(0) # [1, 16, 577, 577]
            # image_attentions_x = image_attentions_x[:, :, 0, :, 0, 1:].mean(dim=2)
            # if os.path.exists('attentions.pt'):
            #     # Load the attentions from the file
            #     image_attentions_all = torch.load('attentions.pt')
            #     image_attentions_all = torch.cat((image_attentions_all, image_attentions_x), dim=0)
            #     torch.save(image_attentions_all, 'attentions.pt')
            #     print(image_attentions_all.shape)
            # else:
            #     # Save the attentions to a file
            #     image_attentions_all = image_attentions_x
            #     torch.save(image_attentions_all, 'attentions.pt')
            #     print(image_attentions_all.shape)
